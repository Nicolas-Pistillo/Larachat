...
<html lang="{{ str_replace('_', '-', app()->getLocale()) }}">
    <head>
        ...
        <title>Bienvenido a Larachat</title>
        ...
        <link rel="stylesheet" href="{{ asset('css/custom/welcome/welcome.css') }}">
        ...
        <header class="" x-data="{ isOpen: false }">
            <nav class="fixed top-0 container px-6 py-4 mx-auto md:flex md:justify-between md:items-center">
                ...
                <div :class="isOpen ? 'flex' : 'hidden'"
                    ...
                    @if (Route::has('login'))
                            @auth
                                <a href="{{ url('/dashboard') }}" class="px-8 py-2 text-sm font-medium text-white transition-colors duration-300 transform bg-green-600 rounded hover:bg-green-500">Volver al dashboard</a>
                            @else
                                <a href="{{ route('login') }}" class="text-sm font-medium text-gray-200 transition-colors duration-300 transform hover:text-indigo-400">Iniciar sesion</a>

                                @if (Route::has('register'))
                                    <a href="{{ route('register') }}" class="text-sm font-medium text-gray-200 transition-colors duration-300 transform hover:text-indigo-400">Registrarse</a>
                                @endif
                            @endauth
                    @endif
                </div>
        ...
        <div class="max-w-5xl px-6 py-16 mx-auto">
            ...
            <div class="grid gap-8 mt-10 md:mt-20 md:grid-cols-2">
                <div class="flex space-x-4">
                    <svg class="w-6 h-6 text-gray-500" viewBox="0 0 50 50" fill="#444" xmlns="http://www.w3.org/2000/svg">
                        <path
